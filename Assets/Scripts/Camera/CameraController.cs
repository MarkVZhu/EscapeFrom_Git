...
public class CameraController : MonoBehaviour
{
    ...
    private float purposeRotation = 180f;
    ...
    {
        SetCameraPosition();
        if (GameObject.Find("GSpace"))
            this.rotateCamera();
    }

    void rotateCamera()
    {
        if (GameObject.Find("GSpace").GetComponent<GravityChange>().canRotate)
        {
            transform.rotation = Quaternion.RotateTowards(transform.rotation, Quaternion.Euler(0, 0, purposeRotation), 100 * Time.deltaTime);

            if (this.transform.rotation.z == -1f)
            {
                GameObject.Find("GSpace").GetComponent<GravityChange>().canRotate = false;
